...
function MiniHistogram({
  ...
  // Constrói bins com boa resolução: 12 bins entre min e max.
  const min = Math.min(...intervals);
  const max = Math.max(...intervals);
  const nBins = Math.min(12, Math.max(4, Math.floor(Math.sqrt(intervals.length))));
  ...
  const paddingX = 8;
  const paddingY = 6;
  const barW = (w - paddingX * 2) / nBins;
  return (
    <svg viewBox={`0 0 ${w} ${h + 16}`} className={className} width="100%">
      {bins.map((b, i) => {
        const bh = maxCount === 0 ? 0 : ((b.count / maxCount) * (h - paddingY));
        ...
        return (
          <g key={i}>
            ...
            >
              <title>
                {formatTimeShort(b.from)} → {formatTimeShort(b.to)} — {b.count}
              </title>
            </rect>
            <text
              x={x + barW / 2}
